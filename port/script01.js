...
// 只要 localStorage 沒拿到 key，isAdmin 就會是 false
const isAdmin = computed(() => {
    return USER_KEY.value && USER_KEY.value.trim() !== "";
});
...
const tempUser = ref(""); // 用於設定頁面輸入
const tempKey = ref("");  // 用於設定頁面輸入
    ...
    try {
        const api = `${CATALOG_SERVICE_URL}?action=listMyTrips&user=${identity.value.user}&key=${identity.value.key}`;
        const res = await fetch(api).then(r => r.json());
        if (res.success) {
            myTrips.value = res.trips;
        }
    } catch (e) {
        console.error("同步行程清單失敗", e);
    } finally {
        isSyncingTrips.value = false;
    }
};

const saveAndSyncIdentity = () => {
    if (!tempUser.value || !tempKey.value) {
        showToast("請填寫完整帳號與金鑰", "error");
        return;
    }
    identity.value = { 
        user: tempUser.value.toLowerCase().trim(), 
        key: tempKey.value.trim() 
    };
    localStorage.setItem('travel_pro_identity', JSON.stringify(identity.value));
    showToast("身分已登錄，正在同步清單");
    fetchMyTrips();
};

const switchTrip = (trip) => {
    // 切換時直接跳轉網址並帶入該行程的 Admin Key
    window.location.href = `index.html?trip=${trip.trip_id}&key=${trip.admin_key}`;
};
...
const submitForm = async () => {
    ...
    fetchMyTrips,
                ...
                linkify: (text) => {
    if (!text) return "";
    // 🔧 修正：先跳脫 HTML 特殊字符，避免 </script> 等標籤造成截斷
    const escaped = text
        .replace(/&/g, '&amp;')
        .replace(/</g, '&lt;')
        .replace(/>/g, '&gt;')

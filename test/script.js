...
createApp({
    setup() {
        ...
        const prevPhoto = () => {
            if (zoomScale.value > 1.1) return; // 縮放時鎖定切換
        ...
        onMounted(() => {
            ...
            window.addEventListener('touchstart', (e) => {
                // 如果在燈箱、彈窗或橫向捲動區，不觸發分頁切換
                if (lightboxUrl.value || showModal.value || e.target.closest('.overflow-x-auto') || e.target.closest('textarea')) return;
